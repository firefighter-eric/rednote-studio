...
function usage() {
  console.error(`Usage:
node write_xhs_content.mjs --notes NOTES.json --mapping MAPPING.json --base-dir data/food_notes/items [--dry-run]

NOTES.json:
  [{"title":"小红书卡片标题","url":"https://www.xiaohongshu.com/user/profile/...","postedAt":"2026-05-01T12:00:00+08:00","views":1000,"likes":88,"favorites":12}]
...
function writeMeta(file, dirName, notes) {
  ...
  const engagementBlock = [
    "engagement:",
    engagementNotes.length ? "  source: 'xiaohongshu'" : "  source: 'unavailable'",
    engagementNotes.length
      ? "  note: '这些数据来自小红书页面或传入的 notes JSON。'"
...
async function main() {
  ...
  for (const [dirName, titles] of Object.entries(mapping)) {
    ...
    const firstPostedAt = titles
      ...
      .sort()[0];
    const targetDirName = firstPostedAt && !/^\d{4}-\d{2}-\d{2}_/.test(dirName)
